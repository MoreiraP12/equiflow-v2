"""
Test script for eICU dataset analysis with EquiFlow.
Note: Requires eicu_all_patients.csv file to run.
"""
...
def test_eicu_flow():
    """Test EquiFlow with eICU patient data."""
    
    # Check if data file exists
    if not os.path.exists("eicu_all_patients.csv"):
        print("eicu_all_patients.csv not found. Skipping test.")
        return
    ...
    ef_full.add_exclusion(
        keep=ef_full._dfs[-1]['non_cardiac_patient'] != 0,
    ...
    ef_full.add_exclusion(
        keep=ef_full._dfs[-1]['septic_patient'] != 0,
        exclusion_reason="sepsis diagnosis",
        new_cohort_label="Patients with sepsis"
    )

    ef_full.add_exclusion(
        keep=~ef_full._dfs[-1]['max_troponin'].isna(),
