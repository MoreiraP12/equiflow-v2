"""
Test script for label formatting in EquiFlow.
"""
...
ef_full.add_exclusion(
    keep=~ef_full._dfs[1]['race'].isna(),
    exclusion_reason="Missing race data",
    new_cohort_label="Complete Race Data"
)

ef_full.add_exclusion(
    keep=(ef_full._dfs[2]['bmi'] >= 15) & (ef_full._dfs[2]['bmi'] <= 50),
...
ef_focused.add_exclusion(
    keep=~ef_focused._dfs[1]['race'].isna(),
    exclusion_reason="Missing race data",
    new_cohort_label="Complete Race Data"
)

ef_focused.add_exclusion(
    keep=(ef_focused._dfs[2]['bmi'] >= 15) & (ef_focused._dfs[2]['bmi'] <= 50),
    exclusion_reason="BMI outside range 15-50",
    new_cohort_label="Final Analysis Cohort"
)
